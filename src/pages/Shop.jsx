...
const Shop = () => {
  ...
  <div className="flex justify-between w-full">
    ...
    <div className="flex items-center space-x-6">
      <img src="/images/filtering.png" alt="Filter Icon" className="w-5 h-5" />
      ...
      <span className="text-[#000000] text-base font-normal">Showing 1-26 of 32 results</span>
    </div>

    
    <div className="flex items-center space-x-4 pr-[45px]">
      <span className="text-[#000000] text-xl font-normal">Show</span>
      <div className="px-4 py-1 bg-white border  rounded-md text-[#9F9F9F] text-xl font-normal">16</div>
      ...
      {loading ? (
        ...
        {shopProduct.map((product, index) => (
          <div key={index} className="flex flex-col items-start bg-white p-4 rounded-lg ">
            <img
              src={product.img}
              alt={product.title}
            ...
            <div className=" w-full bg-gray-200 p-4 rounded-lg">
              <h3 className="text-[#3A3A3A] text-2xl font-semibold">{product.title}</h3>
              <span className="text-[#898989] font-medium text-base block">{product.span}</span>
              <p className="text-[#3A3A3A] mt-2 text-xl font-semibold">{product.price}</p>
            </div>
          </div>
        ))}
      </div>
  ...
  {icons.map((icon, index) => (
    <div key={index} className="flex items-center gap-4">
      ...
      <div className="flex flex-col">
        <h3 className="text-2xl  font-semibold text-[#242424]">{icon.title}</h3>
        <span className="text-xl font-medium text-[#898989]">{icon.span}</span>
      </div>
    </div>
  ))}
</div>
